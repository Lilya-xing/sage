...
class SAGEController:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.debug:
            print(
                f"🚀 Starting SAGE Controller for Feature {self.feature_id} "
                f"at Layer {self.layer}"
            )
        ...
        try:
            while not self.state_machine.is_final_state():
                if time.time() >= deadline:
                    raise TimeoutError(
                        f"SAGE exceeded timeout budget of "
                        f"{self.timeout_seconds / 60.0:.2f} minutes"
                    )
                if self.execution_stats["total_rounds"] >= max_iterations:
                    raise RuntimeError(
                        f"SAGE exceeded execution budget of {max_iterations} iterations"
                    )
                self._execute_round()
