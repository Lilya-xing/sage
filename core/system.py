...
class System:
    ...
    def _process_batch(self, batch_texts: List[str]) -> List[FeatureActivationResult]:
        ...
        try:
            ...
            raise RuntimeError("Failed to compute SAE activation batch") from e
    ...
    def get_activation_trace(self, text: str) -> Dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError(
                "Local activation mode requires a loaded model, tokenizer, and SAE; "
                "synthetic fallback activations are disabled."
            )
        ...
        with torch.no_grad():
            if self.use_hooked_transformer:
                ...
                if self.layer >= len(hidden_states):
                    raise ValueError(
                        f"Layer {self.layer} not available; model returned "
            ...
            if self.sae and "__sae_lens_obj__" in self.sae:
                try:
                    ...
                    raise RuntimeError(
                        f"SAE encoding failed for feature {self.feature_index}"
                    ) from e
                ...
                raise RuntimeError(
                    "No compatible SAE encoder is loaded; hidden-state norms are "
                    "not valid SAE feature activations."
                )
