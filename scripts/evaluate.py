...
def generative_activation_threshold(exemplars: List[Dict[str, Any]]) -> float:
    """Paper metric: half the maximum activation among the top exemplars."""
    activations = [float(item["activation"]) for item in exemplars]
    if not activations:
        raise ValueError("At least one exemplar activation is required")
    if not all(math.isfinite(value) for value in activations):
        raise ValueError("Exemplar activations must be finite")
    return 0.5 * max(activations)


def mean_per_example_pearson(
    examples: List[Tuple[List[float], List[float]]]
) -> Dict[str, Any]:
    """Compute Pearson per held-out example, then average valid correlations."""
    correlations = []
    for predicted, actual in examples:
        if len(predicted) != len(actual):
            raise ValueError("Predicted and actual activation lengths must match")
        if len(predicted) < 2:
            continue
        if np.var(predicted) == 0 or np.var(actual) == 0:
            continue
        correlation, _ = pearsonr(predicted, actual)
        if not np.isnan(correlation):
            correlations.append(float(correlation))
    if not correlations:
        return {"correlation": None, "valid_examples": 0, "correlations": []}
    return {
...
def call_neuronpedia_api(
    ...
    if force_regenerate:
        raise ValueError(
            "Remote explanation regeneration/deletion is disabled in the "
    ...
    matching = next(
        (
            ...
            if item.get("explanationModelName") == explanation_model_name
            and item.get("typeName") == explanation_type
        ),
        None,
    )
    ...
    if matching is None:
        available = [
            {
                "explanationModelName": item.get("explanationModelName"),
...
def generate_examples_from_explanation(
    ...
    feature_description = explanation.get("description", "").strip()
    if not feature_description:
        raise ValueError(
            f"{source} explanation has no canonical description; refusing "
            "label-only or mismatched evaluation"
        )
    print(f"explanation: {feature_description}")
    explanations = feature_description
...
def evaluate_prediction_ability(
    ...
    try:
        for i, example_text in enumerate(examples, 1):
            ...
            # Retain pooled values for diagnostics, but score each held-out
            # example independently as specified by the paper.
            all_predicted_activations.extend(predicted_values)
            all_actual_activations.extend(actual_values)
            per_example_pairs.append((predicted_values, actual_values))
        ...
        metric = mean_per_example_pearson(per_example_pairs)
        correlation = metric["correlation"]
        per_example_correlations = metric["correlations"]
        valid_examples = metric["valid_examples"]
        p_value = np.nan
        pred_variance = np.var(pred_array)
        actual_variance = np.var(actual_array)

        if correlation is None:
            print("   📊 Mean per-example Pearson correlation: unavailable")
            correlation = np.nan
        else:
            print(
                f"   📊 Mean per-example Pearson correlation: "
                f"{correlation:.4f} across {valid_examples} examples"
            )
        ...
        result = {
            ...
            'skipped': False,
            'correlation_valid': not np.isnan(correlation),
            'per_example_correlations': per_example_correlations,
            'valid_correlation_examples': valid_examples,
            ...
            'method': 'mean_per_example_logprobs_token_level',
...
def process_single_feature(
    ...
    try:
        ...
        if all_api_exemplars and len(all_api_exemplars) > 0:
            ...
            if not high_exemplars:
                raise RuntimeError("No high-activating exemplars found")
            max_activations = [
                float(exemplar.get("activation", 0.0))
                for exemplar in high_exemplars
            ]
            dynamic_activation_threshold = generative_activation_threshold(
                high_exemplars
            )
            print(
                f"   ✅ Paper threshold = 0.5 × top-10 maximum: "
                f"{dynamic_activation_threshold:.4f}"
            )
            print(
                f"      Max activation range: "
                f"[{min(max_activations):.4f}, {max(max_activations):.4f}]"
            )
            ...
            raise RuntimeError("No Neuronpedia exemplars retrieved")
    except Exception as e:
        raise RuntimeError(
            "Failed to obtain exemplars required for paper metrics"
        ) from e
    ...
    print("\n" + "="*80)
    print("STEP 2: Fetch Existing Neuronpedia Explanation")
    print("="*80)
    # Baseline retrieval is read-only. Existing data is snapshotted locally.
...
def main():
    ...
    print(f"        (half of the maximum activation). API failure aborts evaluation.")
    print(f"\nPaths:")
    ...
    print("\n" + "="*80)
    if error_count:
        print(f"❌ EVALUATION FAILED: {error_count} feature(s) errored")
        print("="*80)
        return 1
